...
export function EducatorCRMPanel({ isArabic, desktopRuntime = false }: Props) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
      ...
      <article className="surface-panel rounded-2xl p-5"><div className="flex items-center gap-3"><FileText className="h-5 w-5 text-sky-100/70" /><p className="text-display text-3xl">{labels.essays}</p></div><div className="mt-6 grid grid-cols-3 gap-2 text-center">{[["Idea", "03"], ["Draft", "05"], ["Review", "02"]].map(([label, value]) => <div key={label} className="rounded-xl border border-white/10 p-3"><p className="text-2xl text-display">{value}</p><p className="mt-1 text-[10px] uppercase tracking-[0.12em] text-white/40">{label}</p></div>)}</div><div className="mt-5 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full w-2/3 rounded-full bg-sky-100/75" /></div><p className="mt-3 text-xs leading-5 text-white/45">{isArabic ? "من فكرة أولية إلى تغذية راجعة ثم نسخة معتمدة، مع دليل واضح لكل تغيير." : "From idea to feedback to approved version, with evidence for each revision."}</p></article>
    </div>
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {[{ icon: HeartHandshake, title: labels.mentoring, ar: "أمل · 3 جلسات موثقة", en: "Amal · 3 documented sessions" }, { icon: MessageSquareText, title: labels.behavior, ar: "مشاركة إيجابية · متابعة واحدة", en: "Positive participation · 1 follow-up" }, { icon: Library, title: labels.resources, ar: "24 مورداً معتمداً", en: "24 approved resources" }, { icon: TrendingUp, title: labels.evolution, ar: "A2 → B1 خلال 2 فصل", en: "A2 → B1 across 2 terms" }].map(({ icon: Icon, title, ar, en }) => <button key={title} onClick={() => toast.info(isArabic ? "سيفتح سجل هذه الوحدة في النسخة التالية." : "This module record opens in the next iteration.")} className="surface-panel rounded-2xl p-5 text-right transition hover:-translate-y-0.5 hover:border-white/25"><Icon className="h-5 w-5 text-white/55" /><p className="mt-7 text-xl text-display">{title}</p><p className="mt-2 text-sm text-white/70">{isArabic ? ar : en}</p><p className="mt-2 text-[11px] leading-5 text-white/40">{isArabic ? "سجل قابل للمراجعة، لا قرار آلي." : "Reviewable record, no automated decision."}</p></button>)}
    </div>
    <div className="rounded-2xl border border-amber-200/15 bg-amber-200/5 p-4 text-xs leading-6 text-amber-50/70"><ClipboardList className="mr-2 inline h-4 w-4" />{isArabic ? "ملاحظة تنفيذية: هذه الواجهة تربط سير العمل. ربطها النهائي بسجلات المؤسسة يتطلب نقل الطلاب والمهام من التخزين المحلي إلى جداول المؤسسة المقيّدة." : "Implementation note: this interface connects the workflow. Final record binding requires moving students and tasks from local storage into institution-scoped tables."}</div>
  </div>;
}
