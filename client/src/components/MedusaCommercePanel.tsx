...
export default function MedusaCommercePanel({ isArabic }: Props) {
  ...
  const createInvoice = trpc.commerce.createInvoice.useMutation({ onSuccess: () => { setLearnerId(""); setProductId(""); setInvoiceAmount(""); setDiscount(""); toast.success(isArabic ? "تم إصدار الفاتورة." : "Invoice issued."); void utils.commerce.invoices.invalidate(); }, onError: (error) => toast.error(error.message) });
  const recordPayment = trpc.commerce.recordInvoicePayment.useMutation({ onSuccess: () => { toast.success(isArabic ? "تم تسجيل الدفعة." : "Payment recorded."); void utils.commerce.invoices.invalidate(); }, onError: (error) => toast.error(error.message) });
  const changeInvoiceStatus = trpc.commerce.updateInvoiceStatus.useMutation({ onSuccess: () => { toast.success(isArabic ? "تم تحديث حالة الفاتورة." : "Invoice status updated."); void utils.commerce.invoices.invalidate(); }, onError: (error) => toast.error(error.message) });
  ...
  return <div className="space-y-6">
    ...
    <section className="surface-panel rounded-2xl p-6">
      ...
      <form className="mt-5 grid gap-3 md:grid-cols-[1fr_1fr_0.7fr_auto]" onSubmit={(event) => { event.preventDefault(); if (!learnerId || !productId) return toast.error(isArabic ? "اختر الطالب والخدمة." : "Choose a learner and service."); const product = localProducts.data?.find(item => item.id === productId); const discountMinor = Math.round(Number(discount || 0) * 100); if (product && discountMinor > product.amountMinor) return toast.error(isArabic ? "الخصم أكبر من قيمة الخدمة." : "Discount exceeds service amount."); createInvoice.mutate({ learnerId, productId, discountMinor }); setInvoiceAmount(product ? String(product.amountMinor / 100) : invoiceAmount); }}>
        ...
        <input min="0" step="0.01" type="number" value={discount} onChange={(event) => setDiscount(event.target.value)} placeholder={isArabic ? "خصم دج" : "Discount DZD"} className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-3 text-sm text-white outline-none" />
