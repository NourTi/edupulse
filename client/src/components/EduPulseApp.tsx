...
function HexBadge({ number, title, copy, icon: Icon }: HexBadgeProps) {
  return <article className="group relative flex min-h-[190px] items-center justify-center px-8 py-9 text-center transition duration-200 hover:-translate-y-1" style={{ clipPath: "polygon(25% 3%, 75% 3%, 98% 50%, 75% 97%, 25% 97%, 2% 50%)", background: "linear-gradient(145deg, rgba(255,255,255,0.17), rgba(255,255,255,0.045))" }}>
    <div className="absolute inset-[1px] flex items-center justify-center px-8 py-9" style={{ clipPath: "polygon(25% 3%, 75% 3%, 98% 50%, 75% 97%, 25% 97%, 2% 50%)", background: "linear-gradient(180deg, rgba(0,54,74,0.98), rgba(0,38,54,0.98))" }}>
      <div><div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full border border-amber-100/30 bg-amber-100/10 text-amber-100"><Icon className="h-4 w-4" /></div><p className="mt-3 text-[10px] tracking-[0.22em] text-white/35">{number}</p><h3 className="text-display mt-1 text-2xl leading-tight text-white">{title}</h3><p className="mt-2 text-xs leading-5 text-white/55">{copy}</p></div>
    </div>
  </article>;
}
...
export default function EduPulseApp() {
  ...
  const landingNav = [
    ["benefits", "الفوائد"], ["sis", "SIS"], ["platform", "المنصة"], ["roles", "الأدوار"], ["assistant", "المساعد"], ["local", "محلي وآمن"],
  ];
  ...
  if (screen === "landing") {
    return <main className="bg-[hsl(201_100%_13%)] text-white" dir={direction}>
      ...
      <section id="benefits" className="border-t border-white/10 bg-[#002b3c] px-6 py-24 sm:px-8"><div className="mx-auto max-w-7xl"><SectionHeader eyebrow={isArabic ? "لماذا EduPulse؟" : "Why EduPulse"} title={isArabic ? <>إشارات صغيرة.<br /><em className="not-italic text-white/55">أثر كبير في يوم المدرسة.</em></> : <>Small signals.<br /><em className="not-italic text-white/55">A larger school-day effect.</em></>} copy={isArabic ? "بطاقات مرئية تلخّص القيمة التي يلمسها المدير والمعلم وولي الأمر والطالب." : "Visual callouts for the value administrators, educators, guardians, and learners feel every day."} /><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">{[
        ["01", isArabic ? "فعّال التكلفة" : "Cost effective", isArabic ? "تقليل الأدوات المتفرقة." : "Fewer disconnected tools.", WalletCards],
        ["02", isArabic ? "تقييم الأداء" : "Performance evaluation", isArabic ? "دليل واضح للتقدم." : "Evidence for progress.", BarChart3],
        ["03", isArabic ? "بيانات موحّدة" : "Unified records", isArabic ? "سجل واحد لكل متعلم." : "One record per learner.", Database],
        ["04", isArabic ? "تواصل أسرع" : "Faster communication", isArabic ? "مسودات ورسائل مفهومة." : "Clear drafts and messages.", MessageCircle],
        ["05", isArabic ? "خصوصية عملية" : "Practical privacy", isArabic ? "صلاحيات وعزل مؤسسي." : "Roles and tenant isolation.", ShieldCheck],
        ["06", isArabic ? "ذكاء موثّق" : "Grounded intelligence", isArabic ? "إجابات مع مصادر." : "Answers with sources.", Sparkles],
      ].map(([number, title, copy, Icon]) => <HexBadge key={String(number)} number={String(number)} title={String(title)} copy={String(copy)} icon={Icon as typeof Database} />)}</div></div></section>
  ...
  const dashboardTitle = ({ overview: "صباح واضح.", registration: "تسجيل طالب جديد.", learners: "سجل الطلاب.", subjects: "مكتبة المواد الدراسية.", attendance: "حضور اليوم.", cefr: "تقدم اللغة الإنجليزية.", guardians: "تواصل إنساني واضح.", payments: "مدفوعات وإيصالات.", reports: "تقارير التقدم.", knowledge: "دليل المؤسسة.", ask: "اسأل المؤسسة.", crm: "نظام المعلم.", portal: "بوابة الطالب." } as Record<string, string>)[activeView] ?? "EduPulse";
