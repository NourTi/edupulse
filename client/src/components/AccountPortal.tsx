...
type Props = { language: AccountLanguage; onBack: () => void; onAuthenticated: () => void };
type Mode = "login" | "register" | "invite" | "forgot" | "reset";

const copy = {
  ar: { login: "تسجيل الدخول", register: "إنشاء مؤسسة", invite: "قبول دعوة", forgot: "نسيت كلمة المرور", reset: "إعداد كلمة مرور جديدة", email: "البريد الإلكتروني", password: "كلمة المرور", name: "الاسم الكامل", institution: "اسم المؤسسة", token: "رمز الدعوة", submitLogin: "دخول آمن", submitRegister: "إنشاء مساحة المؤسسة", submitInvite: "تفعيل الحساب", submitForgot: "إرسال رابط الاستعادة", submitReset: "حفظ كلمة المرور", switchRegister: "أنشئ مساحة تعليمية جديدة", switchLogin: "لديك حساب؟ تسجيل الدخول", switchInvite: "لديك دعوة من مؤسسة؟", switchForgot: "نسيت كلمة المرور؟", back: "العودة للمنصة", note: "حسابات EduPulse تُدار بكلمة مرور وتُحفظ صلاحياتها داخل المؤسسة.", strength: "10 أحرف على الأقل، حرف كبير، حرف صغير، ورقم.", error: "تعذر إتمام العملية. راجع البيانات وحاول مرة أخرى.", success: "تم تسجيل الدخول.", created: "تم إنشاء مساحة المؤسسة.", activated: "تم تفعيل الحساب.", sent: "إذا كان الحساب موجوداً، سيصل رابط الاستعادة إلى بريدك.", resetDone: "تم تحديث كلمة المرور." },
  en: { login: "Sign in", register: "Create institution", invite: "Accept invite", forgot: "Forgot password", reset: "Set a new password", email: "Email address", password: "Password", name: "Full name", institution: "Institution name", token: "Invitation token", submitLogin: "Secure sign in", submitRegister: "Create institution workspace", submitInvite: "Activate account", submitForgot: "Send recovery link", submitReset: "Save password", switchRegister: "Create a new education workspace", switchLogin: "Already have an account? Sign in", switchInvite: "Have an institution invitation?", switchForgot: "Forgot your password?", back: "Back to platform", note: "EduPulse accounts use passwords and institution-scoped permissions.", strength: "At least 10 characters, uppercase, lowercase, and a number.", error: "The request could not be completed. Check the details and try again.", success: "Signed in.", created: "Institution workspace created.", activated: "Account activated.", sent: "If the account exists, a recovery link will arrive by email.", resetDone: "Password updated." },
};
...
export function AccountPortal({ language, onBack, onAuthenticated }: Props) {
  ...
  const title = mode === "login" ? "مساحة موثوقة." : mode === "register" ? "ابدأ السجل." : mode === "invite" ? "انضم بأمان." : mode === "forgot" ? "نستعيد الوصول." : "كلمة مرور جديدة.";
  return <section className="relative z-10 mx-auto w-full max-w-xl rounded-[2rem] border border-white/20 bg-[#002b3c]/95 p-6 shadow-2xl backdrop-blur-2xl sm:p-9" dir={language === "ar" ? "rtl" : "ltr"}>
    <div className="mb-8 flex items-start justify-between gap-4"><button onClick={onBack} className="inline-flex items-center gap-2 text-xs text-white/70 transition hover:text-white"><ArrowLeft className="h-4 w-4" />{t.back}</button><div className="rounded-full border border-white/20 bg-white/10 p-3 text-white/90"><ShieldCheck className="h-5 w-5" /></div></div>
    <div className="mb-8"><p className="mb-3 text-xs uppercase tracking-[0.2em] text-white/65">EduPulse · {heading}</p><h1 className="text-display text-5xl leading-none text-white">{title}</h1><p className="mt-4 text-sm leading-7 text-white/75">{t.note}</p></div>
