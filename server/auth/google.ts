...
function requestedOrigin(req: Request) {
  const queryOrigin =
    typeof req.query.origin === "string"
      ? normalizeOrigin(req.query.origin)
      : null;
  return (
    queryOrigin ||
...
export function consumeGoogleState(
  ...
  const index = states.findIndex((candidate) =>
    constantTimeEqual(candidate, state)
  );
  if (index < 0) return { valid: false, remaining: states };
  return {
    valid: true,
    remaining: states.filter((_, candidateIndex) => candidateIndex !== index),
  };
}
...
export function registerGoogleRoutes(app: Express) {
  ...
  app.get("/api/auth/google/callback", async (req, res) => {
    ...
    const savedOrigin =
      normalizeOrigin(savedCookies[ORIGIN_COOKIE]) ||
      normalizeOrigin(process.env.APP_BASE_URL) ||
      appBaseUrl(req);
    ...
    if (!state || !stateResult.valid)
      return res
        .status(400)
        .send(restartMessage("Invalid Google sign-in state.", "state_mismatch"));

    if (stateResult.remaining.length) {
      res.cookie(STATE_COOKIE, stateResult.remaining.join("."), {
        httpOnly: true,
        sameSite: "lax",
        secure: cookieSecure(req),
        maxAge: 10 * 60 * 1000,
    ...
    if (!code)
      return res
        .status(400)
        .send(
          restartMessage("Google sign-in was cancelled or failed.", "missing_code")
        );
    ...
    try {
      const tokenResponse = await fetch(GOOGLE_TOKEN, {
        ...
        body: new URLSearchParams({
          ...
          redirect_uri: callbackUrl(savedOrigin),
      ...
      if (!user || user.status !== "active")
        return res
          .status(403)
          .send("This EduPulse account is not active.");
